...
public class Battle {
	...
	public static void start(Player player, Daemon daemon) {
		...
		try (FileWriter log = new FileWriter("battle_log.txt")){
			...
			log.write("先攻は" + first.getName() + "!\n\n");
			
			System.out.println("先攻は" + first.getName() + "!\n");
			...
			while (player.isAlive() && daemon.isAlive()) {
				...
				log.write(first.getName() + "→" + second.getName() + "に攻撃!\n");
			...
			if (player.isAlive()) {
				...
				log.write(player.getName() + "の勝利!\n");
				
			} else {
				
				System.out.println(daemon.getName() + "の勝利!");
				
				log.write(daemon.getName() + "の勝利!\n");
